...
let loads riscv instr = 
  ...
  (* LB *)
  | 0x0l -> 
    riscv.regs.(rd) <- sextd 7 (Mem.load_byte riscv.mem addr)
  (* LH *)
  | 0x1l -> 
    riscv.regs.(rd) <- sextd 16 (Mem.load_half riscv.mem addr)
  (* LW *)
  | 0x2l -> 
    riscv.regs.(rd) <- sextd 32 (Mem.load_word riscv.mem addr)
  (* LD *)
  | 0x3l -> 
    riscv.regs.(rd) <- Mem.load_double riscv.mem addr
  (* LBU *)
  | 0x4l -> 
    riscv.regs.(rd) <- Mem.load_byte riscv.mem addr
  (* LHU *)
  | 0x5l -> 
    riscv.regs.(rd) <- Mem.load_half riscv.mem addr
  (* LWU *)
  | 0x6l -> 
    riscv.regs.(rd) <- Mem.load_word riscv.mem addr
  | _ -> raise RISCV_illegal_instruction
...
let stores riscv instr = 
  ...
  match funct3 instr with
  (* SB *)
  | 0x0l -> Mem.store_byte riscv.mem addr data
  (* SH *)
  | 0x1l -> Mem.store_half riscv.mem addr data
  (* SW *)
  | 0x2l -> Mem.store_word riscv.mem addr data
  (* SD *)
  | 0x3l -> Mem.store_double riscv.mem addr data
  | _ -> raise RISCV_illegal_instruction
